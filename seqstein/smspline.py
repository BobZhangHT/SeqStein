...
class BaseSMSpline(BaseEstimator, metaclass=ABCMeta):
    ...
    @abstractmethod
    def __init__(self, knot_num=10, knot_dist="quantile", reg_gamma="GCV", xmin=-1, xmax=1):
    ...
    def fit(self, x, y, sample_weight=None):
        ...
        if unique_num >= 4:
            ...
            if self.knot_dist == "uniform":
                ...
                #knots = np.percentile(x, list(np.linspace(0, 100, self.knot_num + 2, dtype=np.float32)),
                #                      interpolation="nearest").tolist()[1:-1]
                knots = np.percentile(x, list(np.linspace(0, 100, self.knot_num + 2, dtype=np.float32)),
                                      interpolation="nearest")[1:-1]
                knots = list((knots - self.xmin) / (self.xmax - self.xmin))
                knots = np.unique([0.0] + knots + [1.0]).tolist()
                if self.reg_gamma == "GCV":
                    self.sm_ = stats.smooth_spline(x, y, all_knots=ro.FloatVector(knots),
                                         w=sample_weight, tol=1e-6 * (np.max(x) - np.min(x)))
                else:
